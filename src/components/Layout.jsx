...
import InspectorMark from './InspectorMark'
...
export default function Layout() {
    ...
    <div className="flex h-full flex-col gap-1 overflow-y-auto px-3 py-4">
      <div className="mb-4 flex items-center gap-3 px-2">
        <div className="grid h-10 w-10 place-items-center rounded-xl bg-white shadow-glow">
          <InspectorMark size={26} />
        </div>
        <div className="min-w-0">
          <p className="truncate text-sm font-extrabold text-white">Inspections</p>
      ...
      <NavItem to="/app/schedule" icon={CalendarDays} label="Schedule" badge={dueTodayTasks.length} onClick={close} />

      <p className="px-3 pb-1 pt-4 text-[10px] font-bold uppercase tracking-widest text-ink-500">Manage</p>
      <NavItem to="/app/forms" icon={ClipboardList} label="Inspection Forms" onClick={close} />
      <NavItem to="/app/forms/new" icon={ClipboardCheck} label="Create Form" onClick={close} />

      <p className="px-3 pb-1 pt-4 text-[10px] font-bold uppercase tracking-widest text-ink-500">Track</p>
      <NavItem to="/app/overdue" icon={CalendarClock} label="Overdue" badge={overdueTasks.length} onClick={close} />
      <NavItem to="/app/records" icon={History} label="Records" onClick={close} />

      {isAdmin && (
        <>
          <p className="px-3 pb-1 pt-4 text-[10px] font-bold uppercase tracking-widest text-ink-500">Admin</p>
          <NavItem to="/app/sites" icon={Building2} label="Sites" onClick={close} />
      ...
      <div className="mt-auto pt-4">
        <div className="rounded-xl bg-white/5 p-3">
          <div className="flex items-center gap-2.5">
            ...
            <button onClick={doLogout} className="rounded-lg p-2 text-ink-400 hover:bg-white/10 hover:text-white" title="Sign out">
              <LogOut size={16} />
            </button>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-clay-bg">
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-20 flex items-center gap-3 bg-clay-bg/80 px-4 py-3 backdrop-blur lg:hidden">
          ...
          <span className="flex items-center gap-2 font-extrabold">
            <InspectorMark size={18} className="text-brand-500" /> Inspections
          </span>
        </header>
        ...
        <footer className="no-print mx-auto flex max-w-7xl flex-wrap items-center gap-x-4 gap-y-1 px-4 py-6 text-xs text-ink-400 sm:px-6 lg:px-8">
          <span>© {new Date().getFullYear()} Inspections Portal</span>
          {LEGAL_PAGES.map((p) => (
      ...
      {/* Sam — the floating inspection guide (walks, sleeps, drag-to-pin) */}
      <Assistant />
      ...
      <Modal open={warning} onClose={() => {}} title="Still there?">
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <div className="mt-2 flex w-full gap-2">
            <button className="btn-ghost flex-1" onClick={doLogout}><LogOut size={16} /> Log out now</button>
            <button className="btn-primary flex-1" onClick={stayActive}>Stay signed in</button>
          </div>
        </div>
      </Modal>
    </div>
