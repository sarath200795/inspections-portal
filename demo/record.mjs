...
const outFile = path.join(outDir, 'inspections-portal.mp4')
const silentFile = path.join(outDir, 'inspections-portal.silent.mp4')
...
const t0 = Date.now()
const rec = await startScreencast(page, path.join('/tmp', 'frames-inspections-portal'))
initAudio(path.join('/tmp', 'audio-inspections-portal'), t0)
...
if (res) {
  ...
  const wav = path.join('/tmp', 'audio-inspections-portal', 'narration.wav')
